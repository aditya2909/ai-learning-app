        ...
        <nav className="flex gap-2">
          {tabs.map((tab) => (
            <button
              ...
              className={`relative pb-4 px-6 text-sm font-semibold transition-all duration-300 ${activeTab === tab.name ? "text-emerald-600" : "text-slate-600 hover:text-slate-900"}`}
            >
              ...
              {activeTab === tab.name && (
                <div className="absolute inset-0 bg-linear-to-b from-emerald-50/50 to-tranparent rounded-t-xl -z-10" />
              )}
            </button>
          ))}
        </nav>
