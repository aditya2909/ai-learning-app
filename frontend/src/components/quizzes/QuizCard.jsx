...
const QuizCard = ({ quiz, onDelete }) => {
  return (
    <div className="group relative bg-white/80 bakdrop-blur-xl border-2 border-slate-200 hover:border-emerald-300 rounded-2xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/10 flex flex-col justify-between">
      <button
        onClick={(e) => {
          e.stopPropagation();
          onDelete(quiz);
      ...
      <div className="space-y-4">
        ...
        <div className="inline-flex items-center gap-1.5 py-1 rounded-lg text-xs font-semibold">
          <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-1 ">
            <Award strokeWidth={2.5} className="w-3.5 h-3.5 text-emerald-600" />
      ...
      <div className="mt-2 pt-4 border-t border-slate-100">
        {quiz?.userAnswers?.length > 0 ? (
            ...
            <Link to={`/quizzes/${quiz._id}`}>
                <button className="group/btn relative w-full h-11 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 overflow-hidden active:scale-95">
